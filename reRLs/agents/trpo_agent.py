...
from reRLs.policies import GaussianPolicyTRPO
from reRLs.infrastructure.buffers import TRPOBuffer
...
class TRPOAgent(BaseAgent):
    ...
    def _create_policy_and_buffer(self):
        ...
        # policy
        self.policy = GaussianPolicyTRPO(
            ...
            max_kl=self.agent_config['max_kl'],
            backtrack_coeff=self.agent_config['backtrack_coeff'],
            max_backtracks=self.agent_config['max_backtracks'],
            num_critic_update_steps_per_train=self.agent_config['num_critic_update_steps_per_train']

        )
        # replay buffer
        self.replay_buffer = TRPOBuffer()
    ...
    def train(self, data_batch) -> Dict:
        ...
        log_pi_old, act_dist_old, advs, q_values = \
                data_batch['obs'], data_batch['act'], data_batch['rew'], \
                data_batch['next_obs'], data_batch['done'], data_batch['log_pi'], \
                data_batch['act_dist_old'], data_batch['adv'], data_batch['q_value']
        ...
        train_log = self.policy.update(
            obss, acts, log_pi_old, act_dist_old, advs, q_values
        )
    ...
    def sample(self, batch_size):
        return self.replay_buffer.sample_random_data(batch_size, agent=self)
    ...
    def get_act_dist(self, obss):
        return self.policy._get_act_dist(obss)
